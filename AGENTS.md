...
- `docs/adr/003-session-reliability-model.md` — accepted decision on who owns live-session state and what reconnect restores: facilitator-authoritative rounds, a values-free submission roster in the snapshot, acknowledged submissions with a kind-driven retry policy, stable client identity, and role-asymmetric link state. **Read before working #9, #50, #51, #60, #61, #62, or #63** — it is the design anchor for all of them. Current sequencing lives on the Epic-0010 sub-issue list (see [Picking the next task](#picking-the-next-task)), not in the ADR.
