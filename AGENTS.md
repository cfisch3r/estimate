...
- **`/calc` is pure and framework-free** — no React, no I/O, no network/storage imports. Functions there should be thoroughly unit-tested (100% statement/branch coverage is enforced in CI via `pnpm test:coverage`'s `vite.config.ts` threshold — `testHelpers.ts`'s defensive-throw branch is the one excluded exception). `pnpm deadcode` (knip) also runs in CI to catch exported-but-uncalled code anywhere in `src/` before it can hide from that threshold behind a barrel re-export.
